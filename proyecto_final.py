...
mk("%s/__RESULTADOS__/CLASS"%ruta)
mk("%s/__RESULTADOS__/STAND"%ruta)
mk("%s/__RESULTADOS__/CLASS/CARTAS_CLASS"%ruta)
mk("%s/__RESULTADOS__/STAND/CARTAS_STAND"%ruta)

pathC="%s/__RESULTADOS__/CLASS/Lista_seleccionados.html"%ruta
pathI="%s/__RESULTADOS__/STAND/Lista_seleccionados.html"%ruta
...
n=30
pot=[0 for v in range(0,n+1)]
fdp=[0 for v in range(0,n+1)]
...
lista_lineas=[0 for v in range(31)]
for i in range (0,31):
...
def escribir_carta(nom_archivo,nombre,fdp,coste,tamor):
		...
		nom_archivo.write("""<p><div align=right>Le&oacute;n, %s </div><br><H4>Estimado Se&ntilde;or/Se&ntilde;ora:<br></H4>Nos ponemos en contacto con usted para informarle de los  beneficios economicos que  obtendria por la mejora del factor de potencia de su instalacion '%s'. Su f.d.p actual es %.2f para mejorarlo hasta %.2f con una instalacion auxiliar de condensadores o inductores, necesitaria una inversion inicial para la compra e instalacion de los equipos de %.2f&euro;. El tiempo de amortizacion seria de %.2f meses, teniendo en cuenta la cantidad mensual que ahorraria en su factura nueva con respecto a la anterior. No tenga reparos en contactar con nosotros para mas detalles. <br><br><br>Reciba un cordial saludo.<br></p>"""%(date,nombre,fdp,fdpmax,coste,tamor))
...
def crear_lista(path):
	f=op(path,"w")
	f.write("""<body leftmargin="50" rightmargin = "300" topmargin="60" ><meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><div align=left>Le&oacute;n, %s </div><br><br><table><tr><th>NOMBRE_EMPRESA_______</th><th>TELEFONO_______</th><th>POTENCIA(KW)_______</th><th>FDP_______</th><th>COSTE(&euro;)_______</th><th>TIEMPO<br>AMORTIZACION(meses)</th></tr>"""%date)
	f.close()
...
def add_to_list(path,nombre,tel,pot,fdp,coste,tamor):
	f=op(path,"a")
	f.write("<tr><td>%s</td><td>%s</td><td><div align=center>%.2f</div></td><td>%.2f</td><td>%.2f</td><td><div align=center>%.2f</div></td></tr>"%(nombre,tel,pot,fdp,coste,tamor))
	f.close()
...
for i in L2:
	crear_cartas("%s/__RESULTADOS__/STAND/CARTAS_STAND"%ruta,"f",i[0],i[3],i[4],i[5])
...
class consumidores_seleccionados(consumidor):
	...
	def __init__(self,nom,tel,pot,fdp):
		...
		self.coste=float(self.pot)*self.fc
		self.tamor=float(self.coste)/self.am
		add_to_list(pathC,self.nom,self.tel,self.pot,self.fdp,self.coste,self.tamor)
		crear_cartas("%s/__RESULTADOS__/CLASS/CARTAS_CLASS"%ruta,"f",self.nom,self.fdp,self.coste,self.tamor)

	@staticmethod
	@time_count_logging
	def seleccion():
		for i in range(len(c)):
			if (c[i].fdp > 0.30 and c[i].fdp < 0.81 and c[i].pot > 80.00):
...
print("**************************************************************************************************************************************")
print ("\nEn la carpeta RESULTADOS ubicada en %s encontrara el archivo txt con la lista total de clientes y un archivo html con los datos de los clientes seleccionados y una carpeta CARTAS con la carta de notificacin generada para cada cliente"%ruta)
print("\n**************************************************************************************************************************************")
